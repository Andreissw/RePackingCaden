...
^PQ{Num.Value},0,1,Y^XZ";
        ...
        int CheckCountInDateD51(string Date)
        {
            ...
            using (var fas = new FASEntities())
            {
                return fas.M_CadenaID.Where(c => c.LOTID == LotID & c.Liter == "D51" & c.IsPacked != null & c.LabelDate == D).Count();
            }
        }
        ...
        string GetDate()
        {
            ...
            using (var fas = new FASEntities())
            {
                var LBDate =  fas.M_CadenaID.Where(c => c.LOTID == LotID & c.Liter == "D51").Select(c => c.LabelDate).Max();                
        ...
        void UpdatePacking(string Trid, int i, string lang)
        {
            using (var fas = new FASEntities())
            {
            ...
            print(LabelSN(Trid, "BarTon TH-562", DateTime.Parse(LabelDate.Text).ToString("dd.MM.yyyy")));
        ...
        List<string> GetFreeSN()
        {
            using (var fas = new FASEntities())
            {
                return fas.M_CadenaID.Where(c => c.LOTID == null & c.IsUsed == false).Select(c => c.TRID).Take(CountSerial).ToList();
            }
        }
